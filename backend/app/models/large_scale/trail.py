...
class TrailInfo(BaseModel):
    ...
    slope_lvl: Optional[str] = Field(None, description="경사 등급 (평지, 완만, 경사)")
    slope_val: Optional[str] = Field(None, description="경사 수치 (0-7%, 7-15% 등)")
    slope_avg: Optional[float] = Field(None, description="평균 경사도 수치 (%)")
    
    # 5. 편의시설 전용 필드 (선택)
    pg_holidays: Optional[str] = Field(None, description="놀이터 휴무일")
    pg_agency: Optional[str] = Field(None, description="놀이터 운영기관")
    pg_phone: Optional[str] = Field(None, description="전화번호")
    pg_size: Optional[str] = Field(None, description="놀이터 규모(㎡)")
    pg_night_light: Optional[str] = Field(None, description="야간조명 여부")
    pg_location: Optional[str] = Field(None, description="시설 위치(주소)")
    pg_fee: Optional[str] = Field(None, description="시설 이용요금")
    pg_hours: Optional[str] = Field(None, description="운영시간")
    pg_notes: Optional[str] = Field(None, description="기타 특이/제약사항")
    pg_floor: Optional[str] = Field(None, description="바닥재(놀이터)")
    pg_large_dog: Optional[str] = Field(None, description="대형견 출입 여부")
